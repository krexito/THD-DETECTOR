...
import MasterBrain from "../components/NLSSummer";
import { useAudioEngine, computeMasterTHD } from "../lib/useAudioEngine";

const INITIAL_CHANNELS: ChannelData[] = [
  { id: "ch1", name: "KICK",   thd: 0.12, thdN: 0.14, level: 75, peakLevel: 80, harmonics: [0.08, 0.04, 0.02, 0.01, 0.005, 0.002, 0.001], muted: false, soloed: false, color: "#f97316" },
  { id: "ch2", name: "SNARE",  thd: 0.08, thdN: 0.09, level: 65, peakLevel: 70, harmonics: [0.05, 0.08, 0.06, 0.03, 0.01, 0.005, 0.002],  muted: false, soloed: false, color: "#60a5fa" },
  { id: "ch3", name: "BASS",   thd: 0.35, thdN: 0.38, level: 80, peakLevel: 85, harmonics: [0.25, 0.12, 0.05, 0.02, 0.01, 0.005, 0.002],  muted: false, soloed: false, color: "#a78bfa" },
  { id: "ch4", name: "GTR L",  thd: 0.22, thdN: 0.25, level: 60, peakLevel: 65, harmonics: [0.15, 0.1, 0.07, 0.04, 0.02, 0.01, 0.005],    muted: false, soloed: false, color: "#34d399" },
  { id: "ch5", name: "GTR R",  thd: 0.19, thdN: 0.22, level: 58, peakLevel: 63, harmonics: [0.12, 0.09, 0.06, 0.03, 0.015, 0.008, 0.004], muted: false, soloed: false, color: "#2dd4bf" },
  { id: "ch6", name: "KEYS",   thd: 0.05, thdN: 0.06, level: 55, peakLevel: 58, harmonics: [0.03, 0.02, 0.01, 0.005, 0.002, 0.001, 0.0005], muted: false, soloed: false, color: "#fbbf24" },
  { id: "ch7", name: "VOX",    thd: 0.07, thdN: 0.09, level: 70, peakLevel: 75, harmonics: [0.05, 0.03, 0.015, 0.007, 0.003, 0.001, 0.0005], muted: false, soloed: false, color: "#f472b6" },
...
export default function Home() {
  ...
  const [worstChannel, setWorstChannel] = useState<string | null>(null);
  ...
  const handleChannelUpdate = useCallback(
    (updates: Map<string, Partial<ChannelData>>) => {
      setChannels((prev) => {
        ...
        const { thd, thdN, worstChannel: worst } = computeMasterTHD(next);
        setMasterThd(thd);
        setMasterThdN(thdN);
        setWorstChannel(worst);
  ...
  useAudioEngine(channels, handleChannelUpdate);
  ...
  return (
    <main
      ...
      style={{
        background: "linear-gradient(180deg, #040810 0%, #060b14 100%)",
    ...
    >
      ...
      <div
        ...
        style={{
          background: "linear-gradient(180deg, #080d16 0%, #040810 100%)",
